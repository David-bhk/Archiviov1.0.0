...
- SQLite reste la source active et le moteur par défaut tant que les parcours applicatifs n'ont pas été validés sur PostgreSQL. Une sélection explicite par environnement permet de lancer l'application sur la copie PostgreSQL sans altérer SQLite ; toute valeur de fournisseur inconnue bloque le démarrage. La base PostgreSQL locale ne reçoit encore aucune écriture de l'application hors opération de copie contrôlée.
