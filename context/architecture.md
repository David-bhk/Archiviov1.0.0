...
- La procédure locale de sauvegarde et de restauration est vérifiée. Treize documents utilisent maintenant la racine gérée : les deux fichiers initiaux et onze cibles externes copiées sous des noms UUID, contrôlées par empreinte SHA-256 puis rattachées transactionnellement après une sauvegarde restaurée avec succès. Les fichiers sources ont été conservés. Les 12 anciens chemins `/uploads/...` issus du seed ont été retirés transactionnellement après une décision explicite et une sauvegarde vérifiée ; les activités associées ont été conservées. Onze métadonnées portant des chemins absolus externes absents restent sans cible confirmée. L'instantané post-nettoyage contient et restaure 24 métadonnées et les 13 fichiers gérés. La politique d'exploitation, le chiffrement et la destination hors machine restent à définir.
