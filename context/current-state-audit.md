...
Les risques serveur directement applicables étaient concentrés dans Express et ses transitives `body-parser`, `path-to-regexp` et `qs`, Multer, Nano ID, ainsi que `jws` via `jsonwebtoken`. La tranche du 29 août 2026 met à jour Express vers 4.22.2, Multer vers 2.2.0, Nano ID vers 5.1.16 et `jsonwebtoken` vers 9.0.3, puis actualise `path-to-regexp` vers 0.1.13 dans le verrou. Les transitives corrigées incluent `body-parser` 1.20.6, `qs` 6.15.3 et `jws` 4.0.1.
...
Après cette maintenance compatible, `npm audit --omit=dev` ne signalait plus aucune vulnérabilité connue. L'audit complet conservait 6 alertes (`3 moderate`, `1 high`, `2 critical`), toutes rattachées au groupe Vite/Vitest et à leurs transitives `esbuild`, `vite-node` et `@vitest/mocker`.

La migration majeure coordonnée du 15 septembre 2026 met ensuite Vite à jour vers 7.3.6, Vitest et `@vitest/coverage-v8` vers 4.1.11, le plugin React vers 5.2.0 et les plugins Replit vers 0.6.1 et 0.0.6. `@types/node` passe à 20.19.43 pour respecter le contrat de Vite 7 sans changer la cible de types Node. L'arbre npm, TypeScript, les 36 tests, la couverture V8 et le build de production réussissent. Les audits complet et `--omit=dev` ne signalent désormais aucune vulnérabilité connue. Aucun `npm audit fix`, `--force` ou `--legacy-peer-deps` n'a été utilisé ; toutes les versions ont été sélectionnées explicitement après vérification de leurs moteurs et dépendances de pair.
...
**État : partiellement résolu.** La racine de stockage est configurée, les nouveaux chemins sont relatifs, les noms physiques utilisent un UUID et toute résolution hors racine est refusée. Les tests couvrent la traversée et les chemins absolus externes. Les contraintes techniques actuellement appliquées par Multer — extensions autorisées et taille maximale de 10 Mio — sont désormais centralisées et partagées avec l'interface de téléversement, sans les présenter comme une décision produit définitive. Après restauration de contrôle d'une sauvegarde préalable, les 11 cibles externes encore présentes ont été copiées sous des noms UUID, vérifiées par empreinte SHA-256 puis rattachées dans une transaction PostgreSQL ; les sources ont été conservées. Après décision explicite, les 12 métadonnées sans fichier issues du seed ont ensuite été retirées transactionnellement, sans supprimer les activités associées. L'audit post-nettoyage confirme 24 métadonnées, 13 fichiers gérés cohérents, aucun fichier géré manquant, aucun ancien chemin `/uploads/...` et aucun doublon physique. Les 11 chemins absolus externes absents sont désormais conservés et signalés comme indisponibles : les réponses API retirent `filePath`, calculent `isAvailable` à partir du chemin, de la nature et de la taille du fichier, et la route de téléchargement applique la même vérification. L'interface affiche cet état et ne propose pas le téléchargement. La détection du type réel et les limites définitives restent ouvertes.
...
**État : résolu.** Le seed refuse désormais la production et tout fournisseur autre que SQLite, exige la confirmation exacte de `prisma/dev.db` et lit le mot de passe de démonstration depuis une variable locale non versionnée. L'ancien mot de passe connu est explicitement refusé. Les départements et comptes sont réconciliés par `upsert` dans une transaction, avec leurs relations stables, et aucune métadonnée documentaire n'est plus créée sans fichier réel. Après décision explicite et sauvegarde restaurée avec succès, les 12 métadonnées historiques sans cible physique ont été supprimées de PostgreSQL ; les activités associées ont été préservées.
