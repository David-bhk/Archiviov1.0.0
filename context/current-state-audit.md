...
**État : partiellement résolu.** La racine de stockage est configurée, les nouveaux chemins sont relatifs, les noms physiques utilisent un UUID et toute résolution hors racine est refusée. Les tests couvrent la traversée et les chemins absolus externes. Les contraintes techniques actuellement appliquées par Multer — extensions autorisées et taille maximale de 10 Mio — sont désormais centralisées et partagées avec l'interface de téléversement, sans les présenter comme une décision produit définitive. Un audit PostgreSQL exécuté dans une transaction en lecture seule confirme 2 fichiers gérés cohérents, 12 chemins historiques `/uploads/...` sans contenu, et 22 chemins absolus externes dont 11 cibles existent encore avec la taille déclarée. Les 12 anciennes entrées forment 3 groupes physiques répétés quatre fois, soit 9 répétitions, conformément au seed non idempotent observé. Aucun candidat de même nom et taille n'existe ailleurs dans le dépôt. La détection du type réel, les limites définitives et la décision de réconciliation restent ouvertes.
